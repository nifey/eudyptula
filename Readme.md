...
The task descriptions were taken from [Agelastic's repository](https://github.com/agelastic/eudyptula).
...
we've been through all of this before), your id is "".
...
haven't forgotten it, but just to be safe, it's "".
...
to use your id of "" in the Subject line properly.
...
And, in case you happened to forget it, your id is "".
...
For this task:
  ...
  - If the id value matches your id (which of course you know as
    "", then the syscall returns success.  Otherwise it
...
When you build the kernel, pass the "C=1" option to the build, to havesparse run on the .c file before gcc is run.  Depending on the file,
